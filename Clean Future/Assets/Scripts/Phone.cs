...
public class Phone : MonoBehaviour
{
    ...
    private void Update()
    {
        if (!didItFire && Distance(white) < 5 && !isDead) // Ateþlendi mi ve mesaafe 5 den azsa.   
        { 
            didItFire = true;
            StartCoroutine(messageGun());
        }
    ...
    IEnumerator messageGun()
    {
        Instantiate(mesage, transform.position, Quaternion.identity);
